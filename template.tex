...
\begin{itemize}
  ...
  \item \underline{dc:date}. String-literal values conforming to ISO 8601 or the Extended Date/Time Format Specification (EDTF) (https://www.loc.gov/standards/datetime/) are used in that order.
...
\begin{itemize}
\item \underline{erc:who} The value is drawn from dc[terms]:creator, dc[terms]:contributor, or dc[terms]:publisher, in that order. If none of those have values, then use \underline{erc:who ``(:unkn) unknown''}.
\item \underline{erc:what} The value is drawn from dc:title or dc:description, in that order. Note that because local usage always stipulates the use of dc:title, local usage is to draw the value from dc:title.
